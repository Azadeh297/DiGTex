...
def load_cora():
    ...
    # ]
    title = [
        f"{text.split(':', 1)[0].strip()} "
        if ':' in text else text
        for text in cora_data.raw_texts
    ]
    content = [
        f"{text.split(':', 1)[1].strip()}"
        if ':' in text else text
        for text in cora_data.raw_texts
    ]

    # for text in cora_data.raw_texts:
    #      title=f"{text.split(':', 1)[0].strip()}"
    #      content= f"{text.split(':', 1)[1].strip()}"
    #----------------- CORA
    data_name='cora'
...
def load_ogb_products():
    ...
    title=data1['title'].fillna('') .tolist()
    content= (data1['content'].fillna('')).tolist()
    # print("data1['category']=",data1['category'])


    print("len(title)",len(title))
    print("len(content)",len(content))

    return data, title, content, data1['category']
...
def load_product():
    ...
    title = [f'{ti}'for ti in text['title']]
            
    content = [f'{cont}'for cont in (text['content'])]
    # print("text=",texts[0])
    return data, title, content
...
def load_pubmed():
    ...
    # data = dataset[0]
    # print("data",data)
    dataset = torch.load(f"datasets/Pubmed/pubmed2_fixed_sbert.pt")
    # data = dataset[0]
    data = Data(
        n_id=torch.arange(dataset.x.shape[0]),
        x=dataset.x,  # ویژگی‌های گره‌ها
        edge_index=dataset.edge_index,  # یال‌ها
        y=dataset.y,  # برچسب‌های گره‌ها
        # train_idx=torch.where(data.train_mask)[0],
        # valid_idx=torch.where(data.val_mask)[0],
        # test_idx=torch.where(data.test_mask)[0],
    ...
    data.train_idx = np.sort(node_id[:int(dataset.num_nodes * 0.6)])
    data.valid_idx = np.sort(
        node_id[int(data.num_nodes * 0.6):int(dataset.num_nodes * 0.8)])
    data.test_idx = np.sort(node_id[int(dataset.num_nodes * 0.8):])
    print("data",data)
    
    # texts = []
    title=[]

    content=dataset.raw_texts
    print("con=",len(content))
    print("con=",type(content))
    content=[]
    title = []
    for t in dataset.raw_texts:
        # print(t)
        # پیدا کردن اولین رخدادِ "abstract" به صورت case-insensitive
        idx = t.find("Abstract")
        if idx == -1:
            # اگر "abstract" نبود، می‌توانیم کل متن را بعنوان عنوان بگذاریم
            title.append(t)
        else:
            title.append(t[:idx].strip())
    for t in dataset.raw_texts:
        # print(t)
        # پیدا کردن اولین رخدادِ "abstract" به صورت case-insensitive
        idx = t.find("Abstract")
...
def load_arxiv_2023():
    ...
    content=[]
    # data.y=df['label']
    # data.y = torch.tensor(df['label'].values, dtype=torch.long)
    for ti, ab in zip(df['title'], df['abstract']):
        # texts.append(f'[sep] {ti}[sep] {ab}')
        title.append(str(ti))
        content.append(str(ab))
    return data, title, content
...
def load_photo():
    ...
    # print("data",data)
    return data, title, texts

# load_photo()
def load_citeseer():
    data = torch.load(f"datasets/citeseer/citeseer_random_sbert.pt")
    print("data", data)
    # print("lanel=", data.label[0])
    # print("y=",data.raw_texts[0])
    # data.y = data.label
